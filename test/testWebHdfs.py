...
cluster = webhdfs.Cluster('sriopmgta0101.recette.local', '50070', 'et20795')


class TestWebHdfs(unittest.TestCase):
    def test_get(self):
        s = webhdfs.curlGet('http://www.google.com')
    ...
    def test_list_dir2(self):
        l = cluster.ls('/apps/qlf/dwh/hive/')
        print(l)
        self.assertTrue(len(l) > 2)

    def test_exist_OK(self):
        self.assertTrue(cluster.existDir('/tmp'))

    def test_exist_KO(self):
        self.assertFalse(cluster.existDir('/ca/cest/sur/ca/n/existe/pas/'))

    def test_list_fake_dir(self):
        try:
            l = cluster._ls('/ca/cest/sur/ca/n/existe/pas/')
            self.fail("did not throw Exception")
        except webhdfs.DirectoryNotFound:
            pass

    def test_list_files(self):
        l = cluster.ls_files('/apps/qlf/dwh/hive/')
        print(l)
        self.assertTrue(len(l) == 0)

    def test_list_dirs(self):
        l = cluster.ls_dirs('/apps/qlf/dwh/hive/')
        print(l)
        self.assertTrue(len(l) > 10)

    def test_join(self):
        self.assertEqual('/cou/cou/cou', cluster.join('/cou', 'cou', 'cou'))
        self.assertEqual('/cou/cou/cou', cluster.join('/cou//', 'cou//', '//cou'))

    def test_mkDir_rmDir(self):
        DIRNAME = 'test_dir####'
        FULLDIRNAME = cluster.join('/tmp', DIRNAME)
        while DIRNAME in cluster.ls_dirs('/tmp'):
            DIRNAME += '#'
        cluster.mkDir(FULLDIRNAME)
        self.assertTrue(DIRNAME in cluster.ls_dirs('/tmp'))
        cluster.rm(FULLDIRNAME)
        self.assertFalse(DIRNAME in cluster.ls_dirs('/tmp'))

    def test_create_file(self):
        cluster.copyFromLocal('toto', '/tmp/test_f')
...
class TestWebHdfsFiles(unittest.TestCase):
    ...
    def testMirror(self):
        ...
        self.listHdfsDir(self.hdfsTmpDir)
        """ CREATING ARBO:
            + self.tmpDir
              - f1
              - f2
        ...
        self.assertGreater(f3_after[webhdfs.FS_MODIFICATION_TIME], f3[webhdfs.FS_MODIFICATION_TIME],
        ...
        self.assertIn('fadded', filesAfter)
        self.assertNotIn('f2', dir1List)
        self.assertIn('dir4', dir1List)
        self.assertNotIn('dir3', dir1List)
        self.listHdfsDir()
    ...
    def listHdfsDir(self, dir=None):
        ...
        for dir in cluster.ls_dirs(dir2):
            self.listHdfsDir(cluster.join(dir2, dir))
